...
class ChineseTokenizer:
    ...
    def __init__(self, base_tokenizer: str = "cl100k_base"):
        """
        初始化分词器
        ...
        # vocab_size 将由 _build_extended_vocab() 方法计算确定
    ...
    def _build_extended_vocab(self):
        ...
        print(f"📊 最终词汇表大小: {self.vocab_size:,} (基础:{self.base_vocab_size:,} + 扩展:{len(self.extended_vocab):,})")
    ...
    def load_vocab(self, filepath: str):
        ...
        if not os.path.exists(filepath):
            print(f"⚠️ 词汇表文件不存在，将创建新的词汇表: {filepath}")
            return False
